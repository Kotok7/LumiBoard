const defaultGlobal = { theme: 'dark', accent: '#4caf50', baseFontSize: '16px' };
...
function buildCalendar() {
  ...
  headers.forEach(h => {
    ...
    if (!slideSettings.calendar.highlightWeekends && (h === 'Sa' || h === 'Su')) {
  ...
  for (let d = 1; d <= days; d++) {
    ...
    if (slideSettings.calendar.highlightWeekends) {
      const wd = (blank + d - 1) % 7;
      if (wd === 5 || wd === 6) c.classList.add('weekend');
    }
    c.onclick = () => {
...
async function fetchWeather() {
  try {
    const url =
      'https://api.open-meteo.com/v1/forecast?latitude=52.23&longitude=21.01&current_weather=true&hourly=relativehumidity_2m,precipitation&daily=weathercode,temperature_2m_max,temperature_2m_min&timezone=auto';
    ...
    if (w.showPressure) mainW.innerHTML += makeCard('Pressure', cw.pressure + ' hPa');
